...
for t = 1:2500
    ...
    %% AGGIORNAMENTO FISICO (Simulazione Continua con ODE45)
    % Definiamo la funzione per ode45: u_opt e L sono costanti in questo intervallo
    ode_dynamics = @(t, x) Car_Like_Model(x, u_opt, L);

    % Integriamo l'ODE nell'intervallo [0, Ts] partendo dallo stato attuale
    % ode45 restituisce la traiettoria temporale tra i campioni
    [t_ode, x_ode] = ode45(ode_dynamics, [0 Ts], x_current);

    % Il prossimo stato "discreto" è l'ultimo punto della traiettoria continua
    x_next = x_ode(end, :)';
...
fprintf('| Velocità Finale (Comando):  %8.4f m/s                      |\n', final_speed);

%% GRAFICA
figure('Color','w', 'Name', 'Simulazione Singola');
...
L_plot = L * 0.4;  
W_plot = L * 0.2;  % Larghezza proporzionata

draw_car = @(x,y,th,col) fill( ...
    x + [cos(th)*-L_plot/2 - sin(th)*-W_plot/2, cos(th)*L_plot/2 - sin(th)*-W_plot/2, cos(th)*L_plot/2 - sin(th)*W_plot/2, cos(th)*-L_plot/2 - sin(th)*W_plot/2], ...
    y + [sin(th)*-L_plot/2 + cos(th)*-W_plot/2, sin(th)*L_plot/2 + cos(th)*-W_plot/2, sin(th)*L_plot/2 + cos(th)*W_plot/2, sin(th)*-L_plot/2 + cos(th)*W_plot/2], ...
